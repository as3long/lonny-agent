...
function highlightTSLine(line: string): string {
  ...
  // Keywords
  const keywords =
    /\b(async|await|const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|new|throw|try|catch|finally|import|export|from|default|class|extends|implements|interface|type|enum|typeof|instanceof|in|of|this|super|yield|static|private|protected|public|readonly|abstract|declare|as|satisfies|null|undefined|true|false|void|never|any|unknown)\b/g
  ...
  // Built-in types
  const builtins =
    /\b(string|number|boolean|symbol|bigint|object|Array|Promise|Map|Set|Record|Partial|Required|Pick|Omit|Exclude|Extract|NonNullable|ReturnType|Parameters|Readonly|console|Error|Date|RegExp)\b/g
...
function highlightJSONLine(line: string): string {
  ...
  line = line.replace(
    /:(\s*)("(?:[^"\\]|\\.)*")/g,
    (_, space, str) => `:${space}${c.string}${str}${c.reset}`,
  )
  // Numbers
  line = line.replace(/\b(\d+\.?\d*)\b/g, m => `${c.number}${m}${c.reset}`)
  // Keywords
...
function highlightShellLine(line: string): string {
  ...
  line = line.replace(
    /^(>?\s*)([\w./-]+)/gm,
    (_, prefix, cmd) => `${prefix}${c.builtin}${cmd}${c.reset}`,
  )
  // Flags
...
function highlightPythonLine(line: string): string {
  ...
  // Keywords
  const keywords =
    /\b(def|class|if|elif|else|for|while|return|import|from|as|with|try|except|finally|raise|pass|break|continue|yield|lambda|self|None|True|False|and|or|not|in|is|async|await)\b/g
  line = line.replace(keywords, m => `${c.keyword}${m}${c.reset}`)
  // Strings
  line = line.replace(/('(?:[^'\\]|\\.)*')/g, m => `${c.string}${m}${c.reset}`)
  line = line.replace(/("(?:[^"\\]|\\.)*")/g, m => `${c.string}${m}${c.reset}`)
...
function highlightYAMLLine(line: string): string {
  ...
  line = line.replace(
    /^(\s*)([\w.-]+)\s*:/gm,
    (_, space, key) => `${space}${c.property}${key}${c.reset}:`,
  )
  // Comments
  line = line.replace(/(#.*)$/g, m => `${c.comment}${m}${c.reset}`)
