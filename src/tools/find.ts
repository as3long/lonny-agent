...
export const createFindTool = (cwd: string): Tool => {
  const definition: ToolDefinition = {
    ...
    parameters: {
      pattern: { type: 'string', description: 'File name pattern to search for (e.g. "*.ts", "test*")', required: true },
      path: { type: 'string', description: 'Directory to search in (default: cwd)', required: false },
      maxResults: { type: 'number', description: 'Maximum number of results (default: 50)', required: false },
    },
  }
